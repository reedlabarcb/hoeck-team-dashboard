...
- [x] Phase 2: Box folder index — **OFFICIALLY STABLE** as of 2026-06-01. End-to-end verified on production after async-walker conversion (P2.15.1–P2.15.5). All 9 E2E steps pass. Final verified walkId `46058047-f6f1-4b1c-87ab-6f8f1e115725` (jobId `b269b3a0-7e03-4e87-8819-38196e7ca9ed`); 27,352 items indexed in 31 min 11 sec at 14.6 items/sec (faster than the original synchronous baseline). Box sync crons were re-enabled in `railway.toml` (P2.15.6) but in an **invalid format — `[[deploy.cronJobs]]` arrays are silently ignored by Railway** (only a single `deploy.cronSchedule` per service runs), so **no Box walk has ever fired autonomously**; the index is only as fresh as the last *manual* walk (2026-06-01). Fix pending = separate Railway cron services (option A); see the "Railway cron" lesson in `docs/LESSONS_LEARNED.md` + the Railway Deployment section. P2.9 (weekly pg_dump → Box) also deferred to a focused mini-phase.
...
- [~] Phase 3: RealNex sync + 4 workflows — **P3.1–P3.4 COMPLETE & LIVE 2026-07-07.** Read-only RealNex→Postgres mirror: safe wrapper (12 GET-only reads, zero write methods — 3-layer enforcement: set-equality test + pre-commit greps + no verb primitive in client), 4-table mirror (`realnex_companies/contacts/groups/sync_jobs`, migrations 0007/0008), in-process patient-worker sync (OData paging + inversion walk for contact→company links), kickoff/status endpoints, `/realnex` UI. First manual sync + idempotency VERIFIED: 1,276 companies / 1,870 contacts / 28 groups / 1,769 links, `rate_limit_hits=0` across ~1,343 calls, zero dupes. Nightly cron CONFIGURED 2026-07-07 but **never actually fired** — the `[[deploy.cronJobs]]` format is silently ignored by Railway (PROJECT-WIDE; box crons never fired either — diagnosed 2026-07-08, see Railway Deployment + LESSONS_LEARNED). Fix pending = separate Railway cron services (option A). Linking is SET-ONLY (see Key Decisions — drift caveat + Rebuild-links remedy). Five RealNex API-shape gotchas found on the watched first sync (see docs/code: OData $top≤100, {value} envelope, PascalCase fields, server paging, GUID case). **Remaining:** create methods (P3.6), Workflows 1–4 (P3.7–P3.11), read UIs (P3.5) — NOT started. P3.10 conversational parser DEFERRED (privacy review). First autonomous cron run did NOT fire 7/8 (root cause: invalid `[[deploy.cronJobs]]` format, project-wide — diagnosed 2026-07-08); **P3.5 (read UIs) remains HELD pending the cron fix (option A: separate Railway services).** rebuildLinks drift remedy audited 2026-07-07 as MIRROR-ONLY: it clears our Postgres `company_key` + re-walks; RealNex stays read-only (client has only `realnexGet`, wrapper still exactly 12 GET methods, set-equality green).
...
- Cron jobs (`railway.toml`): Box incremental daily 4 AM PT (12:00 UTC) `sync:box:incremental`; Box full Sun 5 AM PT (13:00 UTC) `sync:box:full`; RealNex mirror daily 3 AM PT (11:00 UTC) `sync:realnex`. Backup (`backup:weekly`) + PDF `extract-text` crons commented out.
  - ⚠️ **NONE OF THESE HAVE EVER FIRED (diagnosed 2026-07-08).** They use `[[deploy.cronJobs]]` (an array of jobs), which is **not a real Railway feature** — Railway runs only a single `deploy.cronSchedule` per service (executing that service's start command). The array is parsed as inert config (`cronSchedule: null`, `nextCronRunAt: null` in `railway status --json`) and silently never scheduled. Impact: box index stale since the last *manual* walk (2026-06-01); realnex mirror only updated by manual `/realnex` runs. **Fix = separate Railway service per cron job (option A)** — each with its own `cronSchedule` + a start command of `npm run sync:realnex` / `sync:box:incremental` / `sync:box:full` (`scripts/sync-cron.ts` already awaits-then-`process.exit()`, ideal for the one-shot cron model). Env vars are NOT auto-inherited by new services — set per service or via Railway shared variables. `checkBoxMirror` + `checkRealNexMirror` in `/api/health` now expose per-mirror freshness. See `docs/LESSONS_LEARNED.md` "Railway cron".
