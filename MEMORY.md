...
- [x] **P3.11 Master Query (Workflow 4) — COMPLETE & LIVE 2026-07-15 (HEAD `28f7596`).** One `/query` page ("Master Query"; nav "Query" after Contacts) to slice the WHOLE mirror by STACKABLE AND filters, view, and export — **subsumes the original single-purpose lease-report idea** (a lease report is just one query here). Filters (all optional, AND-stacked): entity toggle (companies|contacts — swaps columns + per-entity casing), name/company search, lease-expiration presets (6/12/24mo rolling) + custom from/to, SF min/max, location (city/state/address), 6 type flags (**OR-within-dimension**, AND-across), group. Composable **`buildQueryWhere`** (queries.ts) ANDs only active clauses; per-entity **`queryCols`** resolver isolates the casing asymmetry (company city/state = flat columns; contact city/state = `address->>'City'/'State'`; address-contains = PascalCase `->>` keys; group = `object_groups @> [{Name}]`; flags = boolean cols). NULL `lease_expiry`/`sq_ft` excluded ONLY when that filter is active. One WHERE → two consumers: view (`GET /api/realnex/query`, paginated 100 + `count(*)`) + export (`GET /api/realnex/query/export`, uncapped, 50k unreachable backstop) — BOTH forward EVERY param via one **`parseQueryFilters`** (anti-drift, route-tested). Client-safe pure `lib/realnex/query-filters.ts` (shape/flags/parse/serialize/chips/clearChip/leaseWindow) shared by page+route+tests; removable filter **chips** = single source of truth for what's applied. **3-sheet Excel export** via `scripts/python/query_export.py` (openpyxl, spawned with a temp JSON file like master_excel_read): **Data** (all rows; LXD REAL date cells, SF REAL int cells, Arial, frozen header) / **Quick Reference** (totals + count by flag/group/expiration-month) / **By Topic** (sections by lease-expiration quarter + count+SF subtotals). Export button disabled when empty; export reflects the current filter exactly. All **READ-ONLY; wrapper stays 13; no migration.** Commits `3edf2e5` (query layer) · `8a87fd6` (page + filter UI + view route) · `28f7596` (3-sheet export).
...
- **Last deployed commit:** `28f7596` (2026-07-15, P3.11 Master Query complete) — auto-deploys on every push to main
