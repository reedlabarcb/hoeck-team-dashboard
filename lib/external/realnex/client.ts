 ...
 * CBRE Zscaler — api/app.realnex.com are NOT. See docs/RealNex_API_Discovery.md §2).
 *
 ...
 * │ This module exposes ONLY `realnexGet`. There is deliberately NO generic       │
 * │ verb-taking request function — so no caller can issue PUT / PATCH / DELETE to  │
 * │ RealNex, by accident or otherwise. `realnexPost` (create-only) is added in     │
 * │ P3.6 when Company/Contact/Activity creation lands. A PUT/PATCH/DELETE helper    │
 * │ must NEVER be added here. See docs/PHASE3_BUILD_PLAN.md "RealNex Write Safety". │
 * └─────────────────────────────────────────────────────────────────────────────┘
 */
...
function requireKey(): string {
  const k = process.env.REALNEX_API_KEY;
  if (!k || k.trim().length < 10) throw new RealNexNotConfiguredError();
  return k;
}
